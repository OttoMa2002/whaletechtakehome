...
把结构化登记推送到企业微信群（代表保安端）。**从接通到推送 < 25 秒**，体验像真人门岗。
...
- **关键设计**：开场白一句批量问全；车牌/手机号在最后一次复述里念出供纠正；公司名走租户名单模糊匹配（"金鱼"→"鲸鱼科技"）；四字段齐即提交、写库 + 推企微 + 优雅挂断。每轮 STT/LLM/TTS 延迟与"接通→推送"总耗时都打结构化日志作 25s 达标证据。
