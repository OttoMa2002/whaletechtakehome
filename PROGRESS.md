...
## 当前状态：阶段 0–4 完成（来电源已打通，接收侧音质受限/属外部限制），下一步阶段 5
| 阶段 | 内容 | 状态 |
|---|---|---|
...
| 4 来电源 | BlackHole 虚拟声卡 → **微信语音 + Continuity 真电话两种来电源都端到端全程跑通**（STT/对话/名单/复述/入库/推企微/优雅收尾全在线）。**唯一遗留：访客听到的门岗 TTS 被接收侧 App 的麦克风 DSP 弄糊**（微信、真电话均如此，Continuity 仅略好），属外部限制、非本系统问题（详见「阶段4 音质坑」）。 | ✅ 通路达成（音质受外部限制） |
...
**音质坑（未解决，判定为接收侧 App 的限制，非本系统问题）**：访客听到的门岗 TTS **稀烂/串道/嘈杂**，微信与真电话都如此（Continuity 仅"稍微好一点点"）。排查结论：
- BlackHole 环回测试（自播自录算互相关）：2ch 与 16ch **都 1.000 完美无损** → 我们的管线和声卡链路干净。
- 换设备（2ch↔16ch）、换采样率（24k↔48k）四种组合都烂 → 变量都在我方动过，无效。
- 访客**上行**语音（同样过编码）STT 识别完美 → 语音编码本身不毁音质。
- 结论：毁音质的是**接收侧通话 App 对"麦克风输入"的 DSP**（降噪/AGC/AEC/窄带语音编码，为真人真麦调的），把从虚拟麦灌进去的合成语音当噪声/回声处理。微信尤甚、真电话略好；**在对方 App/运营商侧，我方关不掉**。生产形态走正经电信 SDK（阿里云语音）注入即无此问题。
