...
## 当前状态：阶段 0–5 核心完成（对话压时间+问候开关+README 齐）；留待多轮录像验证与阶段6
| 阶段 | 内容 | 状态 |
|---|---|---|
...
| 5 计时+文档 | 多轮验证<25s、README/架构图、ASR鲁棒性调优 | 🟡 核心完成（对话压时间+问候开关+README 已做；多轮录像验证留待，详见「阶段5 进度」） |
...
- 实测（本地麦）：3 轮、接通→推送 **18.73s 达标 ✅**，写库+推企微+优雅退出全在线。

**留待（之后有空再做）：**
- **多轮 <25s 录像证据**（本人录；干净达标走本地麦那条）。
- 电话/微信多跑几通，看首声触发后 8k 下对话稳定性（"别复述"软指令偶发不服从）。
- 选型文档归档（SELECTION.md → docs/，可选）；两个脚本加固（自动拉 Docker 守护进程，可选）。
- 阶段6 加分项（查询/回访/并发）。
