...
## 当前状态：阶段 0–4 完成；阶段 5 进行中（对话压时间已做，文档/多轮验证待完成）
| 阶段 | 内容 | 状态 |
|---|---|---|
...
| 5 计时+文档 | 多轮验证<25s、README/架构图、ASR鲁棒性调优 | 🟡 进行中（对话压时间已做，详见「阶段5 进度」） |
...
## 阶段5 进度（进行中）
**已做（对话进一步压时间 + 性能，已 commit）：**
- 开场白缩短（~30→21字）；追问缺字段时**不复述已采集信息**（只问缺的那个）。
- **车牌按省份简称封闭集归一**（"互为/呼为/护"→沪 等同音纠正），少一轮重问。
- 公司 high/fuzzy **都直接采用、不单独确认**，并入最后复述。
- **乐观提交**：四字段齐即调 `submit_registration`，**不阻塞等"对"**；复述句仍念出车牌+手机号供当场纠正。
  - ⚠️ **放松了 CLAUDE.md「车牌+手机号必须复述确认后再推送」的严格时序**（产品决策，对齐项目介绍文档的"3轮~15s"期望效果；代价：误识可能先入库/推送再事后纠正）。`prompt.py` 头注释已留痕。
- **铁律**：成功话术（"已通知门岗/放行/慢走"）只能在 `submit_registration` 返回 ok=true 后说——修复了一版"假登记"bug（LLM 偷懒不调工具、凭空宣布成功，导致没写库没推送、优雅退出也失效）。
- 语速：对话 1.15→**1.25**，开场白 1.45→**1.25**。
- **轮次延迟**：Smart Turn 最大静音等待 3s→**0.8s**（`TURN_STOP_SECS` 可调），每轮更跟手。根因：pipecat 默认 3s，8k 劣化音质下模型常拿不准接近上限→每轮卡 2-3s。
- 实测（本地麦，戴耳机）：3 轮、接通→推送 **18.73s 达标 ✅**，写库+推企微+优雅退出全在线。

**待完成：**
- **多轮 <25s 验证 + 录像证据**（本人录；25s 干净达标走本地麦那条最稳）。
- **README**（架构图 + 部署步骤 + env，一页内）、**选型文档归档**（SELECTION.md → docs/）。
- 电话/微信多跑几通，看 8k 下车牌规整 / 乐观提交的实际稳定性。
- 两个脚本加固（可选）：① start.sh/start-local.sh 检测 Docker 守护进程没开就 `open -a Docker` 等就绪再起库（重启电脑后守护进程不自启会踩）；② start-local.sh 把输出切到蓝牙耳机而非强制 MacBook 扬声器（戴耳机时听不到门岗）。
